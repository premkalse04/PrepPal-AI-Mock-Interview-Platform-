...
export const InterviewPin = ({ interview, onMockPage = false }) => {
  ...
  const techStackArray = (interview?.techStack || "")
    .split(",")
    .filter(Boolean); // prevents crash

  const createdDate =
    interview?.createdAt?.toDate
      ? interview.createdAt.toDate()
      : new Date(); // fallback to avoid crashes

  return (
    <Card className="p-4 rounded-md shadow-none hover:shadow-md shadow-gray-100 cursor-pointer transition-all space-y-4">
      <CardTitle className="text-lg">{interview?.position || "No Position"}</CardTitle>
      <CardDescription>{interview?.description || "No Description"}</CardDescription>
